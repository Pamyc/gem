  ...
  <div className={containerClasses}>
    ...
    <div className="flex-1 relative">
      {field.type === "boolean" ? (
        ...
        >
          <option value="Да" className="bg-white dark:bg-[#151923]">
            Да
          </option>
          <option value="Нет" className="bg-white dark:bg-[#151923]">
            Нет
          </option>
        </select>
      ) : isFinancial ? (
        <div className="relative group">
          ...
          <div className="absolute right-0 top-0 bottom-0 flex items-center justify-center w-10">
            ...
            >
              {CURRENCIES.map((c) => (
                <option
                  key={c.value}
                  value={c.value}
                  className="bg-white dark:bg-[#151923] text-gray-900 dark:text-gray-200"
                >
                  {c.label}
                </option>
              ))}
            ...
            <ChevronDown
              size={10}
              className="absolute right-1 top-1/2 -translate-y-1/2 pointer-events-none text-gray-400 opacity-50"
            />
          </div>
        </div>
        ...
        <AutocompleteInput
          ...
          onChange={(val) => {
            if (field.type === "number") {
              ...
              const normalized =
                digits === "" ? "0" : digits.replace(/^0+(?=\d)/, "") || "0";
              onChange(field.db, normalized);
              return;
            }
            onChange(field.db, val);
          }}
          ...
          type="text" // важно: НЕ number, чтобы не было стрелок
          inputMode={field.type === "number" ? "numeric" : undefined} // если AutocompleteInput это поддерживает
        />
      ) : (
        <input
          type="text"
          ...
          onChange={
            field.type === "number"
              ? (e) => {
                  ...
                  onChange(field.db, digitsOnly); // даём пустое во время ввода
                }
              : (e) => onChange(field.db, e.target.value)
          }
          onBlur={
            field.type === "number"
              ? () => {
