...
const LitersList: React.FC<LitersListProps> = ({
  ...
  const normalizeInt = (raw: unknown) => {
    const digits = String(raw ?? "").replace(/\D/g, ""); // только цифры
    if (digits === "") return "0";
    const noLeading = digits.replace(/^0+(?=\d)/, ""); // 00012 -> 12, "0" оставляем
          ...
          {liters.map((liter, idx) => (
            <div key={idx} className="grid grid-cols-[1fr_80px_80px_32px] gap-2 items-center group">
              ...
              <input
                ...
                inputMode="numeric"
                pattern="[0-9]*"
                value={String(liter.elevators ?? "")}
                onChange={(e) => {
                  const digitsOnly = e.target.value.replace(/\D/g, "");
                  onUpdate(idx, "elevators", digitsOnly);
                }}
                onBlur={() => {
                  onUpdate(idx, "elevators", normalizeInt(liter.elevators));
                }}
              ...
              <input
                ...
                inputMode="numeric"
                pattern="[0-9]*"
                value={String(liter.floors ?? "")}
                onChange={(e) => {
                  const digitsOnly = e.target.value.replace(/\D/g, "");
                  onUpdate(idx, "floors", digitsOnly);
                }}
                onBlur={() => {
                  onUpdate(idx, "floors", normalizeInt(liter.floors));
                }}
                className="bg-gray-50 dark:bg-white/5 border-transparent focus:bg-white dark:focus:bg-black/20 focus:border-indigo-500 border rounded px-2 py-1.5 text-xs outline-none transition-all text-center"
                placeholder="0"
              />
